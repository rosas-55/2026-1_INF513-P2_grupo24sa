import { t as _sfc_main$2 } from "./GuestLayout-T9SRmEee.js";
import { n as _sfc_main$3, r as _sfc_main$5, t as _sfc_main$4 } from "./TextInput-BqeOumt3.js";
import { t as PrimaryButton_default } from "./PrimaryButton-be5gxiiU.js";
...
import { ssrGetDynamicModelProps, ssrInterpolate, ssrLooseContain, ssrRenderAttrs, ssrRenderComponent, ssrRenderStyle } from "vue/server-renderer";
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(ssrRenderComponent(_sfc_main$2, _attrs, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(ssrRenderComponent(unref(Head), { title: "Iniciar Sesión" }, null, _parent, _scopeId));
						...
						_push(`<span class="ms-2 text-sm" style="${ssrRenderStyle({ color: "var(--color-text-secondary)" })}"${_scopeId}>Recordarme</span></label></div><div class="mt-4 flex items-center justify-end"${_scopeId}>`);
						if (__props.canResetPassword) _push(ssrRenderComponent(unref(Link), {
							href: _ctx.route("password.request"),
							class: "rounded-md text-sm underline focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:ring-offset-2",
						...
						_push(ssrRenderComponent(PrimaryButton_default, {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) _push(`${ssrInterpolate(unref(form).processing ? "Ingresando…" : "Iniciar sesión")}`);
								else return [createTextVNode(toDisplayString(unref(form).processing ? "Ingresando…" : "Iniciar sesión"), 1)];
							}),
							_: 1
						}, _parent, _scopeId));
						_push(`</div></form>`);
					} else return [
						createVNode(unref(Head), { title: "Iniciar Sesión" }),
						...
						createVNode("form", { onSubmit: withModifiers(submit, ["prevent"]) }, [
							...
							createVNode("div", { class: "mt-4 block" }, [createVNode("label", { class: "flex items-center" }, [createVNode(_sfc_main$1, {
								...
								checked: unref(form).remember,
								"onUpdate:checked": ($event) => unref(form).remember = $event
							}, null, 8, ["checked", "onUpdate:checked"]), createVNode("span", {
								class: "ms-2 text-sm",
							...
							createVNode("div", { class: "mt-4 flex items-center justify-end" }, [__props.canResetPassword ? (openBlock(), createBlock(unref(Link), {
								...
								default: withCtx(() => [createTextVNode(toDisplayString(unref(form).processing ? "Ingresando…" : "Iniciar sesión"), 1)]),
								_: 1
							}, 8, ["class", "disabled"])])
						], 32)
					];
