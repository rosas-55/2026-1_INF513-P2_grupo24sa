...
<template>
    <AuthenticatedLayout><Head title="Inventario" /><div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="overflow-x-auto rounded-lg border dark:border-gray-700"><table class="min-w-full text-sm text-left text-gray-700 dark:text-gray-300"><thead class="bg-gray-50 dark:bg-gray-800"><tr><th class="px-4 py-3">Fecha</th><th class="px-4 py-3">Insumo</th><th class="px-4 py-3">Tipo</th><th class="px-4 py-3">Cantidad</th><th class="px-4 py-3">Costo Unit.</th><th class="px-4 py-3">Valor Total</th></tr></thead><tbody><tr v-for="m in movimientos.data" :key="m.id" class="border-t dark:border-gray-700"><td class="px-4 py-3">{{ m.fecha }}</td><td class="px-4 py-3">{{ m.insumo?.nombre }}</td><td class="px-4 py-3">{{ m.tipo_movimiento }}</td><td class="px-4 py-3">{{ m.cantidad }}</td><td class="px-4 py-3">{{ m.costo_unitario }}</td><td class="px-4 py-3">{{ m.valor_total }}</td></tr><tr v-if="!movimientos.data.length"><td colspan="6" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">Sin movimientos.</td></tr></tbody></table></div>
